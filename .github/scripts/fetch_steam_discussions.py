...
Usage:
    python fetch_steam_discussions.py <APP_ID> <STATE_FILE>

    APP_ID      Steam App ID to monitor, e.g.: 3718870
    STATE_FILE  Path to a file that persists seen topic IDs between runs.

Outputs (to stdout):
    JSON array of {"title": ..., "url": ..., "game_title": ...} objects for
    new topics.  Writes updated seen IDs back to STATE_FILE.
"""
...
DISCUSSIONS_URL = f"https://steamcommunity.com/app/{APP_ID}/discussions/"
...
def fetch_game_title(app_id: str) -> str:
    ...
    try:
        ...
        if app_data.get("success"):
            return app_data["data"].get("name", app_id)
    except Exception:
        pass
    return app_id
...
def main():
    ...
    game_title = fetch_game_title(APP_ID)
